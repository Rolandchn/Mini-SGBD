...
class JoinOperator(IRecordIterator):
    def __init__(self, 
        ...
        """
        Implémente l'algorithme de jointure Page-Oriented Nested Loops
        
        :param relation_r: Première relation à joindre
        :param relation_s: Deuxième relation à joindre
        :param conditions: Conditions de jointure
        :param buffer_manager: Gestionnaire de tampons
        :param disk_manager: Gestionnaire de disque
        """
        ...
        self.page_dir_r = PageDirectoryIterator(relation_r, disk_manager, buffer_manager)
    ...
    def GetNextRecord(self) -> Optional[Record]:
        ...
        while True:
            ...
            if not self.current_page_r:
                ...
                self.current_page_r = DataPageHoldRecordIterator(
                    self.relation_r, page_id_r, self.buffer_manager
                )
            ...
            if not self.current_page_s:
                page_dir_s = PageDirectoryIterator(
                    self.relation_s, self.disk_manager, self.buffer_manager
                )
                page_id_s = page_dir_s.GetNextDataPageId()
                if not page_id_s:
                    return None
                
                self.current_page_s = DataPageHoldRecordIterator(
                    self.relation_s, page_id_s, self.buffer_manager
                )
    ...
    def Reset(self):
        ...
        self.page_dir_r = PageDirectoryIterator(
            self.relation_r, self.disk_manager, self.buffer_manager
        )
        self.current_page_r = None
        self.current_page_s = None
        self.joined_records = []
